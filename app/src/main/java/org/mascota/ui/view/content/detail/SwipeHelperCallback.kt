...
class SwipeHelperCallback : ItemTouchHelper.Callback() {
    ...
    override fun clearView(recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder) {
        curDx = 0f
        prePosition = viewHolder.bindingAdapterPosition //이거 맞나??
        getDefaultUIUtil().clearView(getView(viewHolder))
    }

    override fun onSelectedChanged(viewHolder: RecyclerView.ViewHolder?, actionState: Int) {
        viewHolder?.let {
            curPosition = viewHolder.bindingAdapterPosition // 이거 맞나??
    ...
    override fun getSwipeEscapeVelocity(defaultValue: Float): Float { //조금 빠른 느낌 수정 해줏요...
        return defaultValue * 10
    }

    override fun getSwipeThreshold(viewHolder: RecyclerView.ViewHolder): Float { //무슨 뜻인지 모르겠삼..
        val isClamped = getTag(viewHolder)
    ...
    private fun clampViewPositionHorizontal(
        ...
        val minSwipe: Float = -view.width.toFloat() / 2 //0.3 만큼만 swipe 되도록
        // RIGHT 방향으로 swipe 막기
        val maxSwipe: Float = 0f

        val x = if (isClamped) {
            // View가 고정되었을 때 swipe되는 영역 제한
            if (isCurrentlyActive) dX - clamp else -clamp //뭔말
        } else {
            dX
        }

        return min(max(minSwipe, x), maxSwipe) //여기도 코틀린 식으로 수정 요망
    }
